 ...
 */
function _ga_set_global_admin_on_network_creation( $new_network_id ) {
...
add_action( 'add_network', '_ga_set_global_admin_on_network_creation', 10, 1 );
 ...
 */
function _ga_adjust_networks_admin_url( $url ) {
	if ( ! has_global_admin() ) {
		return $url;
	}

	return add_query_arg( array( 'page' => 'networks' ), global_admin_url( 'admin.php' ) );
}
add_filter( 'edit_networks_screen_url', '_ga_adjust_network_menus', 10, 1 );
