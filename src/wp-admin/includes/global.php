...
function global_step1( $errors = false ) {
	...
	<form method="post">
		...
		if ( is_wp_error( $errors ) ) {
			echo '<div class="error"><p><strong>' . __( 'ERROR: The multinetwork could not be created.', 'global-admin' ) . '</strong></p>';
		...
		$global_name = ( ! empty( $_POST['global_name'] ) && ! in_array( 'empty_global_name', $error_codes ) ) ? $_POST['global_name'] : sprintf( _x( '%s Networks', 'Default global name', 'global-admin' ), get_option( 'blogname' ) );
		$admin_email = ( ! empty( $_POST['email'] ) && ! in_array( 'invalid_email', $error_codes ) ) ? $_POST['email'] : get_option( 'admin_email' );
		?>

		<p><?php _e( 'Welcome to the Multinetwork installation process!', 'global-admin' ); ?></p>
		...
		<h3><?php _e( 'Multinetwork Details', 'global-admin' ); ?></h3>
		<table class="form-table">
			<tr>
				<th scope="row"><?php esc_html_e( 'Server Address', 'global-admin' ); ?></th>
				<td>
					<?php printf(
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'Multinetwork Title', 'global-admin' ); ?></th>
				<td>
					<input name="global_name" type="text" size="45" value="<?php echo esc_attr( $global_name ); ?>" />
					<p class="description">
						<?php _e( 'What would you like to call your multinetwork?', 'global-admin' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Global Admin Email', 'global-admin' ); ?></th>
				<td>
					<input name="email" type="text" size="45" value="<?php echo esc_attr( $admin_email ); ?>" />
					<p class="description">
						<?php _e( 'Your email address.', 'global-admin' ); ?>
					</p>
				</td>
			</tr>
		</table>
	...
	if ( $_POST || ! is_multinetwork() ) {
		...
		<div class="updated inline"><p>
			<?php printf(
				...
				__( 'We recommend you back up your existing %s file.', 'global-admin' ),
		...
		?>
		<p><?php _e( 'The original configuration steps are shown here for reference.', 'global-admin' ); ?></p>
		<?php
	}
	?>
	<ol>
		<li>
			<p><?php printf(
				...
				__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:', 'global-admin' ),
				'<code>wp-config.php</code>',
				 ...
				 */
				'<code>/* ' . __( 'That&#8217;s all, stop editing! Happy blogging.', 'global-admin' ) . ' */</code>'
			); ?></p>
...
define('MULTINETWORK', true);
	...
	if ( ! is_multinetwork() ) { ?>
		<p><?php _e( 'Once you complete these steps, your multinetwork is enabled and configured.', 'global-admin' ); ?></p>
	<?php }
}
endif;
