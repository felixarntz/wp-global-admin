...
get_current_screen()->add_help_tab(
	array(
		'id'      => 'overview',
		'title'   => __( 'Overview' ),
		'content' => '<p>' . __( 'Add User will set up a new user account in the setup and send that person an email with username and password.', 'wp-global-admin' ) . '</p>',
	)
);
...
if ( isset( $_REQUEST['action'] ) && 'add-user' == $_REQUEST['action'] ) {
	...
	if ( is_wp_error( $user_details['errors'] ) && ! empty( $user_details['errors']->errors ) ) {
		...
		if ( ! $user_id ) {
			...
			/**
			 * Fires after a new user has been created via the global user-new.php page.
			 *
			 * @since 1.0.0
			 *
			 * @param int $user_id ID of the newly created user.
			 */
			do_action( 'global_user_new_created_user', $user_id );
...
<?php } ?>
	...
	<?php
	/**
	 * Fires at the end of the new user form in the Global Admin.
	 *
	 * @since 1.0.0
	 */
	do_action( 'global_user_new_form' );
