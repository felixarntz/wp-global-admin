...
if ( ! current_user_can( 'manage_network_options' ) ) {
	wp_die( __( 'Sorry, you are not allowed to manage options for this network.', 'wp-global-admin' ) );
}
...
require_once( GA_PATH . 'wp-global-admin/wp-admin/includes/global.php' );
...
if ( ! global_table_check() && ( ! defined( 'WP_ALLOW_MULTINETWORK' ) || ! WP_ALLOW_MULTINETWORK ) ) {
	wp_die(
		printf(
			...
			__( 'You must define the %1$s constant as true in your %2$s file to allow creation of a Multinetwork.', 'wp-global-admin' ),
...
if ( is_global_admin() ) {
	$title = __( 'Global Setup', 'wp-global-admin' );
} else {
	$title = __( 'Create a Multinetwork', 'wp-global-admin' );
}

$parent_file = 'settings.php';

$global_help = '<p>' . __( 'This screen allows you to configure a multinetwork setup which will provide you with a Global Admin panel to add further networks.', 'wp-global-admin' ) . '</p>' .
	'<p>' . __( 'The next screen for Global Setup will give you individually-generated lines of code to add to your wp-config.php file. Make a backup copy of that file.', 'wp-global-admin' ) . '</p>' .
	'<p>' . __( 'Add the designated lines of code to wp-config.php (just before <code>/*...stop editing...*/</code>).', 'wp-global-admin' ) . '</p>' .
	'<p>' . __( 'Once you add this code and refresh your browser, multinetwork should be enabled. This screen, now in the Global Admin navigation menu, will keep an archive of the added code. You can toggle between Global Admin and Network Admin by clicking on the Global Admin or an individual network name under the My Networks dropdown in the Toolbar.', 'wp-global-admin' ) . '</p>' .
	'<p><strong>' . __( 'For more information:', 'wp-global-admin' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://github.com/felixarntz/wp-global-admin/wiki/Create-A-Multinetwork" target="_blank">Documentation on Creating a Multinetwork</a>', 'wp-global-admin' ) . '</p>';

get_current_screen()->add_help_tab( array(
	'id'      => 'global',
	'title'   => __( 'Multinetwork', 'wp-global-admin' ),
	'content' => $global_help,
) );
